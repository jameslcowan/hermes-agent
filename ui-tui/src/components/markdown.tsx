...
function MdInline({ sourceOffset, t, text }: { sourceOffset?: number; t: Theme; text: string }) {
  ...
  for (const m of text.matchAll(INLINE_RE)) {
    ...
    if (m[1] && m[2]) {
      ...
      parts.push(
        wrap(renderResolvedLink(parts.length, t, m[4], m[3]), i, matchEnd, false)
      )
    } else if (m[5]) {
      // autolink: rendered url (minus mailto:), not verbatim (has `<>` in source)
      parts.push(wrap(renderResolvedLink(parts.length, t, autolinkUrl(m[5]), m[5].replace(/^mailto:/, '')), i, matchEnd, false))
    } else if (m[6]) {
      ...
      parts.push(wrap(renderResolvedLink(parts.length, t, url), i, urlEnd, true))
...
function MdImpl({ blockIndexBase = 1, cols, compact, msgId, t, text }: MdProps) {
  const blocks = useMemo(() => parseToBlocks(text, compact, cols, t), [cols, compact, t, text])
 ...
 */
function parseToBlocks(text: string, compact: boolean | undefined, cols: number | undefined, t: Theme): MdBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (fence) {
      ...
      if (['md', 'markdown'].includes(lang)) {
        start('paragraph')
        push(
          <Md cols={cols} compact={compact} key={key} t={t} text={block.join('\n')} />,
          blockSource,
    ...
    if (line.includes('|') && i + 1 < lines.length && isTableDivider(lines[i + 1]!)) {
      ...
      push(renderTable(key, rows, t, cols), lines.slice(tableStart, i).join('\n'))
