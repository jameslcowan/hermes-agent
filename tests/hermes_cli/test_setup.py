...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.main.select_provider_and_model", fake_select)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("hermes_agent.cli.auth.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("hermes_agent.cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "hermes_agent.cli.main._model_flow_named_custom",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("HERMES_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", fake_prompt)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "hermes_agent.cli.setup_wizard.get_nous_subscription_features",
    ...
    from hermes_agent.cli.setup_wizard import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("hermes_agent.cli.setup_wizard.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("hermes_agent.cli.setup_wizard._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "hermes_agent.cli.setup_wizard.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_resolve_hermes_chat_argv_falls_back_to_module(monkeypatch):
    ...
    assert setup_mod._resolve_hermes_chat_argv() == [sys.executable, "-m", "hermes_agent.cli.main", "chat"]
