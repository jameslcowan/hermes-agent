...
Validates that _is_network_accessible() correctly classifies addresses and
...
from gateway.platforms.api_server import APIServerAdapter, _is_network_accessible


# ---------------------------------------------------------------------------
# Unit tests: _is_network_accessible()
# ---------------------------------------------------------------------------
...
class TestIsNetworkAccessible:
    ...
    def test_ipv4_loopback(self):
        assert _is_network_accessible("127.0.0.1") is False

    def test_ipv6_loopback(self):
        assert _is_network_accessible("::1") is False
    ...
    def test_ipv4_mapped_loopback(self):
        ...
        assert _is_network_accessible("::ffff:127.0.0.1") is False

    # -- Network-accessible (should return True) --

    def test_ipv4_wildcard(self):
        assert _is_network_accessible("0.0.0.0") is True

    def test_ipv6_wildcard(self):
        # This is the bypass vector that the string-based check missed.
        assert _is_network_accessible("::") is True

    def test_ipv4_mapped_unspecified(self):
        assert _is_network_accessible("::ffff:0.0.0.0") is True

    def test_private_ipv4(self):
        assert _is_network_accessible("10.0.0.1") is True

    def test_private_ipv4_class_c(self):
        assert _is_network_accessible("192.168.1.1") is True

    def test_public_ipv4(self):
        assert _is_network_accessible("8.8.8.8") is True
    ...
    def test_localhost_resolves_to_loopback(self):
        ...
        with patch("gateway.platforms.api_server._socket.getaddrinfo", return_value=loopback_result):
            assert _is_network_accessible("localhost") is False

    def test_hostname_resolving_to_non_loopback(self):
        non_loopback_result = [
            (socket.AF_INET, socket.SOCK_STREAM, 0, "", ("10.0.0.1", 0)),
        ]
        with patch("gateway.platforms.api_server._socket.getaddrinfo", return_value=non_loopback_result):
            assert _is_network_accessible("my-server.local") is True
    ...
    def test_hostname_mixed_resolution(self):
        ...
        with patch("gateway.platforms.api_server._socket.getaddrinfo", return_value=mixed_result):
            assert _is_network_accessible("dual-host.local") is True

    def test_dns_failure_fails_closed(self):
        """Unresolvable hostnames should require an API key (fail closed)."""
        with patch(
            "gateway.platforms.api_server._socket.getaddrinfo",
            side_effect=socket.gaierror("Name resolution failed"),
        ):
            assert _is_network_accessible("nonexistent.invalid") is True
...
class TestConnectBindGuard:
    ...
    def test_allows_loopback_without_key(self):
        ...
        # The guard condition: _is_network_accessible(host) AND NOT api_key
        # For loopback, _is_network_accessible is False so the guard does not block.
        assert _is_network_accessible(adapter._host) is False
    ...
    async def test_allows_wildcard_with_key(self):
        ...
        # The guard checks: _is_network_accessible(host) AND NOT api_key
