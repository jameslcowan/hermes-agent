...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        with patch("hermes_agent.gateway.run._hermes_home", tmp_path), \
             patch("hermes_agent.gateway.run.Path") as MockPath:
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m hermes_agent.cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m hermes_agent.cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "hermes_agent.cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "hermes_agent.cli.main"]
